...
$(document).ready(function () {
    ...
    function readconfig(next) {
        $.ajax({ url: "config.json", dataType: "json" })
    ...
    function init() {
        ...
        var myrequest = $.ajax({ type: "GET", url: csvpath })
            .error(function () {
    ...
    function loaddata(records) {
        ...
        dashboard.dim = {
            main: dashboard.data.dimension(
                oh.utils.get(dashboard.config["item_main"])
            ),
        };
    ...
    function hasdata() {
        if (dashboard.groups.all.value() == 0) {
            alert(
                "Campaign '" +
                    dashboard.campaign_urn +
                    '" has no responses! Try again later (or press F5)'
            );
...
oh.campaign_read = function (cb) {
    var req = oh.call(
        ...
        function (res) {
            if (!cb) return;
            var arg =
                res.metadata && res.metadata.items ? res.metadata.items : null;
            cb(arg);
        }
    );
    return req;
...
oh.getimageurl = function (record) {
    ...
    if (
        !record[photo] ||
        record[photo] == "SKIPPED" ||
        record[photo] == "NOT_DISPLAYED"
    ) {
        return "images/nophoto.jpg";
    }
    var thumbtemplate =
        dashboard.config.photo.image ||
        oh.utils.error("No dashboard.config.photo.image specified");
...
oh.getcsvurl = function () {
    ...
    return decodeURIComponent(
        "/app/survey_response/read?" + jQuery.param(params)
    );
};
(function ($) {
    $.fn.filtermap = function (options) {
        ...
        var mapcontrol = new L.Control.Layers(cloudmaps, {})
            .setPosition("bottomright")
            .addTo(mymap);
        var layercontrol = new L.Control.Layers(
            { Disable: L.layerGroup(), Markers: markerlayer },
            {}
        )
        ...
        $(options.geojson).each(function (index, conf) {
            downloadgeojson(conf.url, function (data) {
                var newlayer = buildgeojsonlayer(data)
                    .setinfo(infobox)
                    .setmap(mymap)
                    .setfilter(conf.item, options.item)
                    .colormap();
                newlayer.setcontrol(
                    layercontrol,
                    conf.title || urltail(conf.url)
                );
    ...
    function buildmarkerlayer(options) {
        ...
        function renderMarkers() {
            ...
            for (var i = 0; i < markerdata.length; i++) {
                ...
                if (!getlat(a)) {
                    dashboard.message(
                        "skipping record " + i + " (no valid latlng)"
                    );
                    continue;
                }
        ...
        function geofilter() {
            ...
            info &&
                info.update(
                    "<pre>" +
                        round(bounds.getSouthWest()["lat"]) +
                        " < lat < " +
                        round(bounds.getNorthEast()["lat"]) +
                        "\n" +
                        round(bounds.getSouthWest()["lng"]) +
                        " < lng < " +
                        round(bounds.getNorthEast()["lng"]) +
                        "</pre>"
                );
            return markerlayer;
        }
    ...
    function buildgeojsonlayer(geojsondata, getlat, getlng) {
        ...
        function hoverin(e) {
            var layer = e.target;
            var infotemplate =
                "<div><h4>Neighborhood</h4> <b> {{ name }} </b></div>";
            ...
            info &&
                info.update(
                    Mustache.render(infotemplate, layer.feature.properties)
                );
        }
        function hoverout(e) {
            ...
            info &&
                info.update(
                    "<div><h4>Neighborhood</h4><i>Hover over map</i></div>"
                );
        }
        function selectNeighborhood(e) {
            selected = e && selected != e.target ? e.target : null;
            if (geodim)
                geodim.filter(selected && selected.feature.properties.name);
            dc.redrawAll();
        }
        ...
        function colormap() {
            ...
            geojson.eachLayer(function (layer) {
                layer.feature.properties.count =
                    countobject[layer.feature.properties.name];
        ...
        function classify(itemgeo, name) {
            ...
            for (var i = 0; i < markerdata.length; i++) {
                ...
                var result = leafletPip.pointInLayer(
                    [getlng(a), getlat(a)],
                    geojson,
                    true
                );
    ...
    function makeinfo() {
        ...
        info.update = function (infotext) {
            this._div.innerHTML =
                infotext ||
                "<div><h4>Load Data</h4><i>Select Data Layer</i></div>";
        };
        return info;
    }
})(jQuery);
    ...
    $.fn.photopanel = function (options) {
        ...
        function updatepictures() {
            ...
            for (var i = 0; i < thispage.length; i++) {
                ...
                var a = $("<a>", { class: "thumbnail", href: "#" }).appendTo(
                    li
                );
                var img = $("<img>", {
        ...
        function updatepages() {
            pages = [];
            currentpage = 0;
            var alldata = oh.utils.getRandomSubarray(
                dashboard.dim.main.top(9999)
            );
        ...
        function getthumburl(record) {
            ...
            if (
                !record[photoItem] ||
                record[photoItem] == "SKIPPED" ||
                record[photoItem] == "NOT_DISPLAYED"
            ) {
                return "images/photothumb.jpg";
            }
            var thumbtemplate =
                options.thumb ||
                oh.utils.error("No photo.thumb specified in config.");
            return Mustache.render(thumbtemplate, record);
        }
    ...
    function calcbin(domain, maxbars) {
        ...
        while (true) {
            for (i = 0; i < k.length; i++) {
                ...
                if (
                    Math.abs(
                        rounddown(domain[0], newbin) -
                            roundup(domain[1], newbin)
                    ) /
                        newbin <
                    maxbars + 1
                ) {
...
(function ($) {
    $.fn.datechart = function (options) {
        ...
        dashboard.dim[dimname] = dashboard.data.dimension(
            oh.utils.getdate(item)
        );
        ...
        mydiv.appendTo(target);
        var mindate = oh.utils.getdate(item)(
            dashboard.dim[dimname].bottom(1)[0]
        );
...
(function ($) {
    $.fn.wordcloud = function (options) {
        ...
        var mydim = (dashboard.dim[variable] = dashboard.data.dimension(
            oh.utils.get(variable)
        ));
        var mydiv = $("<div/>")
            .addClass("wccontainer")
            .addClass("well")
            .css("height", height)
            .appendTo(target);
        var titlediv = $("<div/>").addClass("title").appendTo(mydiv);
...
(function ($) {
    $.fn.responsemodal = function (options) {
        ...
        function showmodal(d) {
            ...
            $("#responsemodal .modal-header h3").text(
                modaltitle.substring(0, 30)
            );
    ...
    function replaceit(x) {
        if (!x || x == "" || x == "NOT_DISPLAYED" || x == "SKIPPED")
            return " - ";
        return x;
    }
})(jQuery);
