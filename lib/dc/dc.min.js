...
dc.units.float.precision = function (precision) {
  var _f = function (s, e, domain) {
    return Math.ceil(Math.abs((e - s) / _f.resolution));
  };
  ...
  if (filter) {
    if (filter instanceof Array) {
      if (filter.length >= 2)
        s =
          "[" +
          printSingleValue(filter[0]) +
          " -> " +
          printSingleValue(filter[1]) +
          "]";
      else if (filter.length >= 1) s = printSingleValue(filter[0]);
    } else {
...
function printSingleValue(filter) {
  ...
  else if (typeof filter == "number") s = Math.round(filter);
  return s;
}
dc.utils = {};
...
dc.utils.GroupStack = function () {
  ...
  function initializeDataPointRow(x) {
    if (!_dataPointMatrix[x]) _dataPointMatrix[x] = [];
  }
  ...
  this.addGroup = function (group, retriever) {
    if (!retriever) retriever = _defaultAccessor;
...
dc.cumulative.CountUnique = function () {
  ...
  this.add = function (key, e) {
    ...
    if (e != null) {
      if (this.getValueByKey(key)[e] == null) this.getValueByKey(key)[e] = 0;
...
dc.baseChart = function (_chart) {
  ...
  _chart.turnOnControls = function () {
    ...
    _chart
      .selectAll(".filter")
      .text(_filterPrinter(_chart.filter()))
      .style("display", null);
    return _chart;
  };
...
dc.coordinateGridChart = function (_chart) {
  ...
  _chart.generateG = function (parent) {
    if (parent == null) _parent = _chart.svg();
    else _parent = parent;
    _g = _parent.append("g");
    _chartBodyG = _g
      .append("g")
...
dc.stackableChart = function (_chart) {
  ...
  _chart.yAxisMin = function () {
    ...
    for (var groupIndex = 0; groupIndex < allGroups.length; ++groupIndex) {
      ...
      var m = dc.utils.groupMin(
        group,
        _chart.getValueAccessorByIndex(groupIndex)
      );
      if (m < min) min = m;
    }
    if (min < 0) {
  ...
  _chart.dataPointHeight = function (d, groupIndex) {
    ...
    var h = 0;
    if (value > 0) h = zeroPosition - yPosition;
    else h = yPosition - zeroPosition;
    if (isNaN(h) || h < MIN_DATA_POINT_HEIGHT) h = MIN_DATA_POINT_HEIGHT;
    return h;
  };
  function calculateDataPointMatrix(data, groupIndex) {
    for (var dataIndex = 0; dataIndex < data.length; ++dataIndex) {
      ...
      if (groupIndex == 0) {
        if (value > 0)
          _groupStack.setDataPoint(
            groupIndex,
            dataIndex,
            _chart.dataPointBaseline() - _chart.dataPointHeight(d, groupIndex)
          );
        else
          _groupStack.setDataPoint(
            groupIndex,
            dataIndex,
            _chart.dataPointBaseline()
          );
      } else {
        if (value > 0)
          _groupStack.setDataPoint(
            groupIndex,
            dataIndex,
            _groupStack.getDataPoint(groupIndex - 1, dataIndex) -
              _chart.dataPointHeight(d, groupIndex)
          );
        else if (value < 0)
          _groupStack.setDataPoint(
            groupIndex,
            dataIndex,
            _groupStack.getDataPoint(groupIndex - 1, dataIndex) +
              _chart.dataPointHeight(d, groupIndex - 1)
          );
        else
          _groupStack.setDataPoint(
            groupIndex,
...
dc.pieChart = function (parent, chartGroup) {
  ...
  function isOffCanvas(current) {
    return (
      current == null || isNaN(current.startAngle) || isNaN(current.endAngle)
    );
  }
  function onClick(d) {
...
dc.barChart = function (parent, chartGroup) {
  ...
  var _chart = dc.stackableChart(
    dc.coordinateGridChart(dc.singleSelectionChart({}))
  );
  ...
  function generateBarsPerGroup(groupIndex, group) {
    var bars = _chart
      .chartBodyG()
      .selectAll("rect." + dc.constants.STACK_CLASS + groupIndex)
  ...
  function barWidth(d) {
    if (_barWidth == null || _chart.refocused()) {
      ...
      else w = Math.floor(_chart.xAxisLength() / numberOfBars);
      w -= _gap;
      if (isNaN(w) || w < MIN_BAR_WIDTH) w = MIN_BAR_WIDTH;
      _barWidth = w;
    }
    return _barWidth;
  }
  ...
  function barX(bar, data, groupIndex, dataIndex) {
    ...
    var position =
      _chart.x()(_chart.keyAccessor()(data)) + _chart.margins().left;
    if (_centerBar) position = position - barWidth(data) / 2;
    return position;
  }
  ...
  _chart.fadeDeselectedArea = function () {
    ...
    if (_chart.isOrdinal()) {
      ...
      else bars.classed(dc.constants.DESELECTED_CLASS, false);
    } else {
      if (!_chart.brushIsEmpty(extent)) {
        var start = extent[0];
        var end = extent[1];
...
dc.lineChart = function (parent, chartGroup) {
  ...
  function plotDataByGroup(groupIndex, group) {
    ...
    if (_renderArea) drawArea(g, stackedCssClass, groupIndex, line);
    if (_chart.renderTitle()) drawDots(g, groupIndex);
  }
  ...
  function drawLine(g, stackedCssClass, groupIndex) {
    ...
    if (linePath.empty())
      linePath = g.append("path").attr("class", "line " + stackedCssClass);
  ...
  function drawArea(g, stackedCssClass, groupIndex, line) {
    ...
    if (areaPath.empty())
      areaPath = g.append("path").attr("class", "area " + stackedCssClass);
    ...
    var area = d3.svg
      ...
      .y0(function (d, dataIndex) {
        ...
        if (y < _chart.dataPointBaseline()) return y - AREA_BOTTOM_PADDING;
        else return y + _chart.dataPointHeight(d, groupIndex - 1);
      });
  ...
  function drawDots(parentG, groupIndex) {
    ...
    if (g.empty()) g = parentG.append("g").attr("class", TOOLTIP_G_CLASS);
    createRefLines(g);
    var dots = g.selectAll("circle." + DOT_CIRCLE_CLASS).data(g.datum());
    dots
      .enter()
      .append("circle")
  ...
  function showRefLines(dot, g) {
    ...
    g.select("path." + Y_AXIS_REF_LINE_CLASS)
      .style("display", "")
      .attr("d", "M" + _chart.margins().left + " " + y + "L" + x + " " + y);
    g.select("path." + X_AXIS_REF_LINE_CLASS)
      .style("display", "")
      .attr(
        "d",
...
dc.rowChart = function (parent, chartGroup) {
  ...
  _chart.doRender = function () {
    _xScale = d3.scale
      .linear()
      .domain([0, d3.max(_chart.group().all(), _chart.valueAccessor())])
  ...
  function drawChart() {
    var rows = _g.selectAll("g." + _rowCssClass).data(_chart.group().all());
  ...
  function updateElements(rows) {
    ...
    var rect = rows
      ...
      .select("rect")
      .attr("height", height);
    dc.transition(rect, _chart.transitionDuration()).attr(
      "width",
      function (d) {
